...
export function handleStreamTokenEvent(data: StreamTokenEvent, ctx: RendererCtx = { messagesEl: null, messagesContainers: new Map(), currentChannelId: null }): void {
  ...
  // 2026-09-22: 真流式 — 每个 token 事件携带的是增量 delta (一小段 content), 必须追加而非替换.
  //   之前的 "replace" 语义是因为旧 loop 每次 emit 的是整段回复前 100 字; 现在 LLM 边生成边推 delta,
  //   追加才能拼成完整回复. thinking 类 (非 token) 同样追加.
