...
- **⑦ 真跑 CLI 抓到的真 bug (已修)**: `bolloon trace` 正常, 但 `bolloon p2p` **输出完了进程不退出**(挂到 400s 超时) —— 根因是它按需 `import network/p2p` 读运行中节点, 而 libp2p 是**常驻模块**(定时器/连接句柄), 一次性信息命令不显式收尾就会一直挂着。修法: `handleP2pCommand` 输出后 `process.exit(ok?0:1)`; 并给 `getLocalP2pInfo()` 加 `BOLLOON_P2P_DEBUG=1` 打点(定位"卡在哪"时不用猜)。复跑: `bolloon p2p` / `bolloon p2p --json` 均 **EXIT=0** 正常收尾, 真读到本机 `did:key:z6MkjW9UCs…` + peerId `12D3KooWHx3tLP…`(落盘记录)。
